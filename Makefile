...
help:
	...
	@echo "  make ensure-login-path - Idempotent: add Go + ~/.local/bin to PATH in shell rc (fixes hugo not found)"
...
# On exit, runs ensure-login-path so ~/.local/bin (Hugo) and GOROOT/bin are on PATH in new shells.
install-go:
	@set -e; \
	trap 'cd "$(CURDIR)" && $(MAKE) ensure-login-path || true' EXIT; \
	GO_ROOT="$(GO_ROOT)"; \
	GV="$(GO_VERSION)"; \
	GO_MIN="$(GO_MIN)"; \
	ARCH="$(HUGO_OS_ARCH)"; \
...
install-hugo:
	...
	@cd "$(CURDIR)" && $(MAKE) ensure-login-path

# Adds GOROOT/bin and ~/.local/bin to PATH in ~/.bashrc / ~/.profile / ~/.zshrc (idempotent).
ensure-login-path:
	@set -e; \
	NEW_MARK='# nolram-blog: local toolchain'; \
	OLD_MARK='# nolram-blog: Go toolchain'; \
	for f in "$$HOME/.bashrc" "$$HOME/.profile" "$$HOME/.zshrc"; do \
		[ -f "$$f" ] || continue; \
		if grep -qF "$$OLD_MARK" "$$f" 2>/dev/null && ! grep -qF '.local/bin' "$$f" 2>/dev/null; then \
			printf '\n# nolram-blog: ~/.local/bin (migration from older Makefile)\nexport PATH="$$HOME/.local/bin:$$PATH"\n' >>"$$f"; \
			echo "Appended ~/.local/bin to PATH in $$f"; \
		fi; \
	done; \
	for f in "$$HOME/.bashrc" "$$HOME/.profile" "$$HOME/.zshrc"; do \
		[ -f "$$f" ] || continue; \
		grep -qF "$$NEW_MARK" "$$f" 2>/dev/null && { echo "Toolchain PATH block already present ($$NEW_MARK)."; exit 0; }; \
	done; \
	for f in "$$HOME/.bashrc" "$$HOME/.profile" "$$HOME/.zshrc"; do \
		[ -f "$$f" ] || continue; \
		if grep -qF "$$OLD_MARK" "$$f" 2>/dev/null && grep -qF '.local/bin' "$$f" 2>/dev/null; then \
			echo "Legacy PATH snippet already includes ~/.local/bin."; \
			exit 0; \
		fi; \
	done; \
	for f in "$$HOME/.bashrc" "$$HOME/.profile" "$$HOME/.zshrc"; do \
		[ -f "$$f" ] || continue; \
		{ echo ""; echo "$$NEW_MARK"; echo 'export GOROOT="$$HOME/.local/go"'; echo 'export PATH="$$GOROOT/bin:$$HOME/.local/bin:$$PATH"'; } >>"$$f"; \
		echo "Appended toolchain PATH to $$f"; \
		exit 0; \
	done; \
	{ echo "$$NEW_MARK"; echo 'export GOROOT="$$HOME/.local/go"'; echo 'export PATH="$$GOROOT/bin:$$HOME/.local/bin:$$PATH"'; } >"$$HOME/.profile"; \
	echo "Created $$HOME/.profile with toolchain PATH"
